...
const ServiceReq = () => {
  ...
  const loadClientVehicles = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7080/api/Clients/vehicle",
        {
          headers: {
  ...
  const loadClientServiceRequests = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7080/api/Clients/service-requests",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
  ...
  const loadServiceInfos = async () => {
    try {
      const response = await axios.get(
        "https://localhost:7080/api/Clients/service-infos",
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
  ...
  const loadFeedbacksForService = async (serviceInfoId) => {
    try {
      const response = await axios.get(
        `https://localhost:7080/api/Clients/feedbacks/service/${serviceInfoId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
      ...
      console.error(
        `Failed to load feedbacks for ServiceInfoId ${serviceInfoId}:`,
        error
      );
    }
  };
  ...
  const handleFeedbackSubmit = async (serviceInfoId) => {
    ...
    try {
      const response = await axios.post(
        "https://localhost:7080/api/Clients/feedbacks",
        feedbackData,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
  ...
  const handleServiceInfoChange = (e) => {
    ...
    const selectedServiceInfo = serviceInfos.find(
      (info) => info.serviceInfoId === parseInt(selectedServiceInfoId)
    ); // Ensure to match the correct ID
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const { serviceRequestId, vehicleId, serviceInfoId, status } =
        response.data;
      setMessage(
        `Service Request added successfully! ID: ${serviceRequestId}, Vehicle ID: ${vehicleId}, Service ID: ${serviceInfoId}, Status: ${status}`
      );
  ...
  const handleDelete = async (requestId) => {
    try {
      await axios.delete(
        `https://localhost:7080/api/Clients/service-requests/${requestId}`,
        {
          headers: {
            Authorization: `Bearer ${localStorage.getItem("token")}`,
            "Content-Type": "application/json",
            ...
            serviceInfos.map((service) => (
              <div key={service.serviceInfoId} className="service-info-card">
                ...
                <div className="provider-info">
                  ...
                  <p className="pN">
                    Name: <span>{service.provider.name}</span>
                  </p>
                </div>
                ...
                <div className="button-container">
                  <button
                    ...
                    className="modal-btnPR"
                  >
                    Provide Feedback
                  </button>
                  <button
                    onClick={() => openFeedbackListModal(service)}
                    className="modal-btnVI"
                  >
                    View Feedback
                  </button>
                </div>
        ...
        {isFeedbackModalOpen && (
          <div className="modalFeed">
            <div className="modal-contentFEed">
              <h2 className="NProviderFeed">
                Provide Feedback for {selectedService?.name}
              </h2>
              <div>
        ...
        {isFeedbackListModalOpen && (
          <div className="modalFeAd">
            <div className="modalcontentENnt">
              ...
              {feedbacks[selectedService.serviceInfoId] &&
              feedbacks[selectedService.serviceInfoId].length > 0 ? (
                <div className="feedback-list">
                  <ul>
                    {feedbacks[selectedService.serviceInfoId].map(
                      (feedback) => (
