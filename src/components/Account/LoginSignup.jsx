...
const Login = () => {
  ...
  const [loginUsername, setLoginUsername] = useState(''); // Changed to loginUsername
  const [loginPassword, setLoginPassword] = useState('');
  const [signupUsername, setSignupUsername] = useState('');
  const [signupEmail, setSignupEmail] = useState('');
  const [signupPassword, setSignupPassword] = useState('');
  const [signupRole, setSignupRole] = useState('Client'); // Default role
  ...
  const handleLoginSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://localhost:7080/api/Account/Login', {
        userName: loginUsername, // Use username instead of email
  ...
  const handleSignupSubmit = async (e) => {
    e.preventDefault();
    try {
      const response = await axios.post('https://localhost:7080/api/Account/Register', {
        userName: signupUsername,
        email: signupEmail,
        password: signupPassword,
        roles: [signupRole] // Selected role
      }, {
        headers: {
          'Content-Type': 'application/json'
        }
          ...
          <form className="form form-login" onSubmit={handleLoginSubmit}>
            <fieldset>
              ...
              <div className="input-block">
                ...
                <input id="login-username" type="text" required value={loginUsername} onChange={(e) => setLoginUsername(e.target.value)} />
              </div>
              <div className="input-block">
          ...
          <form className="form form-signup" onSubmit={handleSignupSubmit}>
            <fieldset>
              ...
              <div className="input-block">
                ...
                <input id="signup-username" type="text" required value={signupUsername} onChange={(e) => setSignupUsername(e.target.value)} />
              </div>
              <div className="input-block">
              ...
              <div className="input-block">
                ...
                <select id="signup-role" value={signupRole} onChange={(e) => setSignupRole(e.target.value)} required>
